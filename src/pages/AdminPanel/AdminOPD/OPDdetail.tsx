...
export default function OPDdetail() {
  ...
  const handleCreateAppointment = (type: any) => {
    if (type === "admin" && (!aptDate || !aptTime)) {
      setError("Fill all fields!");
      return; // Exit function if required fields are not filled
    }

    const localDateTime = `${aptDate} ${aptTime}`;
    const currentDateTime = dayjs().format("YYYY-MM-DD HH:mm:ss");
    const params = {
      appointmentDateAndTime:
        type === "immediately" ? currentDateTime : localDateTime,
    };
    ...
    <div className={classNames(commonStyles.col12)}>
      ...
      <div className={classNames(Styles.mainOuter)}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <div className={classNames(Styles.mt40, Styles.Btnwidth)}>
            <PrimaryButton
              children={"Generate Immediately."}
              colorType={"admin"}
              onClick={() => handleCreateAppointment("immediately")}
            />
          </div>
        </div>
        {showMessageCard && (
                ...
                <div className={Styles.sendOuter}>
                  <div style={{ width: "100px" }}>
                    <PrimaryButton
                      ...
                      onClick={() => handleCreateAppointment("admin")}
                    />
                  </div>
                </div>
              </div>
