...
const Onsite = (props: Partial<Onsite>) => {
  ...
  const FetchAvailibility = () => {
    ...
    DoctorGetAvailibility()
      .then((res: any) => {
        if (res?.status === 200) {
          setPrice(res.data?.availability[0]?.clinicAvailability?.price);
          const apiPhysicalAvailability =
            res.data?.availability[0]?.clinicAvailability?.availability || [];
...
const Availability = (props: Partial<AvailabilityProps>) => {
  ...
  const { handleClickNext, selectedDayData, mode, addData, setAddData } = props;
...
interface ExistingType {
  dayOfWeek: number;
  periods: Array<any>; // Replace 'any' with the actual type of periods array
}
